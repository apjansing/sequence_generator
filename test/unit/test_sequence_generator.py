...
def test_generate_sequence():
    ...
    """
    assert SequenceGenerator([2, 4], filename="test").generate_sequence(5) == [
        2,
        4,
        6,
        10,
...
def test_generate_sequence_with_generator():
    ...
    assert SequenceGenerator(
        [1, 1, 1], generator=lambda x, y, z: x + y + z, filename=None
...
def test_generate_prime_index_sequence():
    ...
    """
    assert SequenceGenerator([2, 3], filename=None).generate_prime_index_sequence(
        5
    ) == [3, 5, 13]
    assert SequenceGenerator([3, 5], filename=None).generate_prime_index_sequence(
        10, True
    ) == [
        (2, 5),
        (3, 8),
    ...
    assert not SequenceGenerator([0, 0], filename=None).generate_prime_index_sequence(0)
...
def test_generate_prime_sequence():
    ...
    element is the number itself. The index is a prime number.
    """
    assert SequenceGenerator([2, 3], filename=None).generate_prime_sequence(5) == [
        2,
        3,
        5,
        13,
    ]
    assert SequenceGenerator([3, 5], filename=None).generate_prime_sequence(
        10, True
    ) == [
        (1, 3),
        (2, 5),
    ...
    assert not SequenceGenerator([0, 0], filename=None).generate_prime_sequence(0)
